package TbWeightResponseMaskMismatch;
...
(* synthesize *)
module mkTbWeightResponseMaskMismatch(Empty);
    ...
    rule consumeRequest(
        ...
        pending <= tagged Valid dut.weightPort.requests.first.tag;
    ...
    rule sendShortMask(isValid(pending));
        ...
        WeightMemoryResponse#(16, 8) response =
            AquaMemoryReadResponse {
                tag: fromMaybe(?, pending),
                payload: ScratchpadRowPayload { mask: mask, data: data }
            };

        dut.weightPort.responses.put(response);
        $display("weight response mask unexpectedly accepted");
        $finish(1);
    endrule
endmodule
