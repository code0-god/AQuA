...
fn maps_second_single_row_stripe_to_manual_qa_coordinates() {
    ...
    let coordinates = (
        ...
        event.k(),
        event.block_index_in_row(),
        event.element_index_in_block(),
    );
...
fn preserves_row_major_residual_event_order() {
    // Given
    let mut row_zero = vec![0; 32];
    row_zero[0] = 200;
    row_zero[3] = 200;
    let mut row_one = vec![0; 32];
    row_one[0] = -200;
    row_one[3] = -200;
    let blocks = [block(row_zero, 1, 0b1001), block(row_one, 1, 0b1001)];
    let config = config(ExsiaPrecision::I8);

    // When
    let result = fold_stripe(&blocks, &config, stripe(0, 0, 2), 32).expect("valid stripe");
