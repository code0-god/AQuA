...
impl TensorDesc {
    // impl: TensorDesc에 함수/메서드를 구현.
    pub fn new(
        dtype: AquaDType, // 함수 인자는 `이름: 타입`.
        shape: Vec<usize>,
    ) -> Result<Self, TensorDescError> {
        // Self = TensorDesc, Result<T, E> = 성공 Ok(T) / 실패 Err(E).
    ...
    pub fn validate(&self) -> Result<(), TensorDescError> {
        let expected = self
            .shape
            .iter()
        ...
        if expected != self.len {
            return Err(TensorDescError::ElementCountMismatch {
                expected,
                actual: self.len,
            });
        }
...
impl fmt::Display for TensorDescError {
    // `impl Trait for Type`: Display trait을 구현.
    fn fmt(
        &self,                              // &self: 현재 값을 immutable reference로 빌림.
        formatter: &mut fmt::Formatter<'_>, // &mut: 수정 가능한 reference, '_는 lifetime 추론.
    ) -> fmt::Result {
        // formatting 성공/실패를 나타내는 Result.

        match self {
            // match: enum variant에 따른 pattern matching.
            Self::ElementCountOverflow =>
            // Self = TensorDescError.
            {
                formatter.write_str("tensor element count overflow")
            }
            Self::ElementCountMismatch { expected, actual } => write!(
                formatter,
                "tensor descriptor element count mismatch: expected {expected}, got {actual}"
            ),
        }
    }
}
...
mod tests {
    ...
    fn validates_descriptor_element_count() {
        ...
        assert_eq!(
            mismatched.validate(),
            Err(TensorDescError::ElementCountMismatch {
                expected: 8,
                actual: 7,
            })
        );
    ...
    #[test]
    fn rejects_descriptor_element_count_overflow() {
        ...
        assert_eq!(
            descriptor.validate(),
            Err(TensorDescError::ElementCountOverflow)
        );
    }
}
