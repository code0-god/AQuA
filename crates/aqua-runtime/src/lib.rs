...
/// before crossing the AQuA runtime boundary.
...
impl HostTensor {
    pub fn f32(
        shape: Vec<usize>,
        data: Vec<f32>,
    ) -> Result<Self, RuntimeError> {          // Self = HostTensor.

        let desc = TensorDesc::new(
            AquaDType::F32,
            shape,
        )?;                                   // TensorDescError는 From을 통해 RuntimeError로 변환됨.
    ...
    pub fn validate(&self) -> Result<(), RuntimeError> {
        ...
        if self.desc.dtype != AquaDType::F32 {
            return Err(
                RuntimeError::UnsupportedDType(self.desc.dtype)
            );
        }

        if self.desc.len != self.data.len() {
            return Err(
                RuntimeError::ElementCountMismatch {
...
pub trait AquaExecutor {                       // 구현체가 따라야 하는 공통 실행 interface.
...
pub enum RuntimeError {
    ...
    ElementCountMismatch {                     // 이름 있는 field를 갖는 struct-like variant.
        expected: usize,
        actual: usize,
    },
}
    ...
    fn fmt(
        ...
        match self {
            Self::Descriptor(error) =>
                error.fmt(formatter),          // 내부 TensorDescError의 Display를 재사용.

            Self::UnsupportedDType(dtype) =>
                write!(
                    formatter,
                    "unsupported host dtype: {dtype:?}"
                ),                             // {:?}: Debug formatting.
...
impl From<TensorDescError> for RuntimeError {  // TensorDescError -> RuntimeError 변환 정의.
